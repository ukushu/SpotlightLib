...
final class SpotlightLibTests: XCTestCase {
    func test_sync_async_call_sameResults() throws {
        let config = SpotlightConfig(daysCount: .last(days: 10),
                                      watchList: watchlist,
                                      ignoredExts: [],
        ...
        let results1 = SpotLight2.getRecentFilesR(config).maybeSuccess!
        ...
        queue.async {
            let results2 = SpotLight2.getRecentFilesR(config).maybeSuccess!
    ...
    func test_absentIgnoredDir() throws {
        let config = SpotlightConfig(daysCount: .last(days: 100),
                                      watchList: watchlist,
                                      ignoredExts: [],
                                      ignoredDirs: ["/Users/uks/Desktop"],
                                      ignoredFiles: []
